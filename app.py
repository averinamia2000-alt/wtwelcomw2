...
openai = AsyncOpenAI(api_key=OPENAI_API_KEY, timeout=60.0, max_retries=1)
...
async def question(message: Message):
    ...
    try:
        ...
        answer = (
            f"Поиск занял больше {REQUEST_TIMEOUT_SECONDS} секунд 😔 "
            "Попробуйте сформулировать вопрос короче или напишите @MiaA_01t — она поможет разобраться."
        )
...
async def main():
    log.info(
        "Starting Whitech Helper v7-fast; space=%s root=%s model=%s timeout=%ss",
