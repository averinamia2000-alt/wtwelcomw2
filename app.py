...
Правила:
- queries: 3–5 КОРОТКИХ вариантов, обычно 1–4 слова.
...
"""
    ...
    plan["queries"] = queries[:5] or [question]
    return plan
...
async def multi_search(plan: dict[str, Any]) -> list[dict]:
    ...
    return sorted(pages.values(), key=lambda x: x["score"], reverse=True)[:10]
...
async def build_answer(question: str) -> str:
    ...
    if not sources:
        return (
            ...
            "Попробуй назвать процесс, отдел, проект или роль другими словами."
        )
...
async def start(message: Message):
    await message.answer(
        ...
        "• Мне нужно уволить сотрудника — что делать?\n"
        "• К кому обратиться по Retention?\n"
        "• Как передать запрос в CC?\n"
        "• Где шаблон устава проекта?\n"
        "• Дай Jira-ссылку по процессу, если она есть в Confluence.\n\n"
        "Я ищу только в базе Whitech (pmprod) и показываю источники."
    )
...
async def main():
    log.info("Starting Whitech bot v3; space=%s model=%s", CONFLUENCE_SPACE_KEY, OPENAI_MODEL)
    try:
