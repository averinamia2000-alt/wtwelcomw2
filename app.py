...
FAQ_BY_ID = {item.get("id"): item for item in FAST_FAQ if item.get("id")}

WEEKLY_REPORT_KEYBOARD = InlineKeyboardMarkup(
    inline_keyboard=[[
        InlineKeyboardButton(text="📊 Global Report", callback_data="weekly:global"),
        InlineKeyboardButton(text="⚙️ Operational Report", callback_data="weekly:operational"),
    ]]
)
...
GUIDED_MENUS = {
    "employee": {
        "text": "Что именно по сотруднику нужно?",
        "buttons": [
            ("👋 Онбординг", "guide:faq:onboarding"),
            ("🧭 Управление", "guide:search:управление сотрудниками"),
            ("👋 Увольнение", "guide:faq:dismissal_jira"),
            ("✍️ Другое", "guide:other"),
        ],
    },
    "traffic": {
        "text": "Что именно по трафику нужно?",
        "buttons": [
            ("📚 База по трафику", "guide:faq:traffic_base"),
            ("🎓 Обучение FAME", "guide:faq:traffic_fame"),
            ("📈 Качество трафика", "guide:search:оценка и качество трафика"),
            ("✍️ Другое", "guide:other"),
        ],
    },
    "kdp": {
        "text": "Какой КДП-вопрос нужно решить?",
        "buttons": [
            ("🏖 Отпуск", "guide:search:отпуск"),
            ("👋 Увольнение", "guide:faq:dismissal_jira"),
            ("📋 КДП-заявка", "guide:faq:kdp"),
            ("✍️ Другое", "guide:other"),
        ],
    },
    "contacts": {
        "text": "Что именно ищем?",
        "buttons": [
            ("👥 ЛПР по отделам", "guide:search:ЛПР по отделам"),
            ("💬 Рабочие чаты", "guide:search:список чатов"),
            ("💻 Сисадмин", "guide:faq:sysadmin_contact"),
            ("✍️ Другой контакт", "guide:other"),
        ],
    },
    "templates": {
        "text": "Какой шаблон нужен?",
        "buttons": [
            ("📝 MFU после встречи", "guide:faq:mfu"),
            ("📄 Устав проекта", "guide:faq:project_charter"),
            ("✅ Чек-лист приёмки", "guide:faq:acceptance_checklist"),
            ("✍️ Другое", "guide:other"),
        ],
    },
}
...
def menu_markup(menu_key: str):
    rows, current = [], []
    for text, data in GUIDED_MENUS[menu_key]["buttons"]:
        current.append(InlineKeyboardButton(text=text, callback_data=data))
        if len(current) == 2:
            rows.append(current)
            current = []
    if current:
...
def guided_menu_for_question(text: str):
    t = re.sub(r"[^a-zа-яё0-9]+", " ", (text or "").lower().replace("ё", "е")).strip()
    words = set(t.split())

    # Only broad/ambiguous formulations go to a menu.
    if any(x in t for x in ["вопрос по сотрудник", "что делать с сотрудник", "про сотрудник"]) and not any(
        x in t for x in ["онборд", "уволь", "фидбек", "конфликт", "грейд"]
    ):
        return "employee"
    if any(x in t for x in ["где про трафик", "вопрос по трафик", "инфа по трафик", "про трафик"]) and not any(
        x in t for x in ["fame", "фейм", "качест", "эконом", "бенчмарк"]
    ):
        return "traffic"
    if ("кдп" in words or "кадров" in t) and not any(x in t for x in ["отпуск", "уволь", "релок"]):
        return "kdp"
    if any(x in t for x in ["нужны контакты", "нужен контакт", "где контакты", "контакты"]) and not any(
        x in t for x in ["сис", "retention", "ретен", "конкрет"]
    ):
        return "contacts"
    if any(x in t for x in ["нужен шаблон", "дай шаблон", "где шаблоны", "какие есть шаблоны", "покажи шаблоны"]) and not any(
...
def is_ambiguous_weekly_report_question(text: str) -> bool:
    item, score, _ = find_fast_faq(text)
    return bool(item and item.get("id") == "weekly_ambiguous" and score >= 0.72)

def weekly_item_by_callback(data: str):
    mapping = {
        "weekly:global": "weekly_global",
        "weekly:operational": "weekly_operational",
    }
    faq_id = mapping.get(data)
    return FAQ_BY_ID.get(faq_id) if faq_id else None
...
@dp.callback_query(F.data == "guide:other")
async def guided_other(callback: CallbackQuery):
    if callback.message:
        await callback.message.edit_text(
            "Опиши вопрос своими словами — попробую найти ответ в базе 🔎"
        )
    await callback.answer()

@dp.callback_query(F.data.startswith("guide:faq:"))
async def guided_faq(callback: CallbackQuery):
    faq_id = (callback.data or "").split(":", 2)[2]
    item = FAQ_BY_ID.get(faq_id)
    if not item:
        await callback.answer("Не получилось открыть этот раздел", show_alert=True)
        return
    answer = render_fast_item(item)
    analytics("guided_button", choice=callback.data, faq_id=faq_id)
    if callback.message:
        await callback.message.edit_text(answer, disable_web_page_preview=True)
    await callback.answer()

@dp.callback_query(F.data.startswith("guide:search:"))
async def guided_search(callback: CallbackQuery):
    query = (callback.data or "").split(":", 2)[2]
    analytics("guided_button", choice=callback.data, query=query)
    if callback.message:
        await callback.message.edit_text(
            f"🔎 Напиши «{query}» отдельным сообщением — найду актуальную информацию в базе."
        )
    await callback.answer()

@dp.callback_query(F.data.in_({"weekly:global", "weekly:operational"}))
async def weekly_report_choice(callback: CallbackQuery):
    item = weekly_item_by_callback(callback.data or "")
    if not item:
        await callback.answer("Не получилось открыть этот отчёт", show_alert=True)
        return
    ...
    if callback.message:
        await callback.message.edit_text(answer, disable_web_page_preview=True)
        history[callback.message.chat.id].append(("Пользователь", f"Выбран {item.get('title') or item.get('id')}"))
        history[callback.message.chat.id].append(("Помогатор", answer[:1200]))
...
def direct_ai_faq(text: str):
    t = re.sub(r"[^a-zа-яё0-9/]+", " ", (text or "").lower().replace("ё", "е")).strip()
    rules = [
        ("ai_security", ["безопасност", "api токен", "api ключ", "парол", "персональн", "production", "autorun", "yolo"]),
        ("ai_code_review", ["code review", "код ревью", "проверить код", "проверять код", "код от ai", "код от ии"]),
        ("ai_plugins_connectors", ["plugin", "плагин", "skill", "скилл", "коннектор", "mcp"]),
        ("ai_goal", ["/goal", "goal clear", "команда goal"]),
        ("ai_model_effort", ["effort", "sonnet", "opus", "haiku", "какую модель", "модель выбрать"]),
        ("ai_automation_examples", ["автоматизац", "внедрени", "что автоматизировать", "идеи для ии", "идеи для ai"]),
        ("ai_chat_rules", ["галлюцинац", "контекст", "один чат", "compact", "как правильно работать", "советы по работе"]),
        ("ai_allowed_tools", ["разрешен", "можно использовать", "список нейросет", "какие нейросет", "какие ai", "какие ии"]),
    ]
    for faq_id, needles in rules:
        if any(n in t for n in needles):
            return FAQ_BY_ID.get(faq_id)
    if any(n in (" "+t+" ") for n in [" ai ", " ии "]) or any(n in t for n in ["нейросет", "chatgpt", "claude", "gemini", "grok", "kimi", "codex"]):
        return FAQ_BY_ID.get("ai_tools_overview")
    return None
...
async def question(message: Message):
    ...
    if ai_item:
        analytics("fast_ai_faq", faq_id=ai_item.get("id"))
        history[message.chat.id].append(("Пользователь", question_text[:800]))
        answer = render_fast_item(ai_item)
        history[message.chat.id].append(("Бот", answer[:1200]))
        await message.answer(answer, disable_web_page_preview=True)
        return

    faq_item, faq_score, faq_method = find_fast_faq(question_text)
    if faq_item and faq_item.get("id") == "weekly_ambiguous":
        analytics(
            "weekly_choice_shown",
            faq_id="weekly_ambiguous",
            match_method=faq_method,
            score=round(faq_score, 3),
        )
        history[message.chat.id].append(("Пользователь", question_text[:800]))
        await message.answer(
            "Какой отчёт нужен?",
            reply_markup=WEEKLY_REPORT_KEYBOARD,
        )
        return

    if faq_item and faq_item.get("id") == "template_ambiguous":
        analytics("guided_menu_shown", menu="templates")
        history[message.chat.id].append(("Пользователь", question_text[:800]))
        await message.answer(
            GUIDED_MENUS["templates"]["text"],
            reply_markup=menu_markup("templates"),
        )
        return

    menu_key = guided_menu_for_question(question_text)
    if menu_key:
        analytics("guided_menu_shown", menu=menu_key)
        history[message.chat.id].append(("Пользователь", question_text[:800]))
        await message.answer(
            GUIDED_MENUS[menu_key]["text"],
            reply_markup=menu_markup(menu_key),
        )
        return
